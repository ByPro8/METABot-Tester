...
"""
Vakifbank Chromium template-check engine (v1) — ExifTool-only.

- Input: ExifTool metadata as structured dict: {Group: {Tag: Value}}
- Templates: meta_templates/vakifbank/*.json
  - ignore: groups/tags
...
Chromium-only:
- This engine is intentionally ONLY for the Chromium/Skia Vakifbank variant.
- iOS/Quartz variant will be implemented as a separate engine later to avoid
  duplicate-key edge cases and special-casing inside one engine.
"""
...
META_TEMPLATES_DIR = BASE_DIR / "meta_templates" / "turkiyefinans"
DEFAULT_TEMPLATE_ID = "TURKIYEFINANS_CHROMIUM_V1"

ALLOWED_TEMPLATE_IDS = ["TURKIYEFINANS_CHROMIUM_V1"]
...
def run_template_check(
    ...
    # Vakifbank Chromium-only engine.
    # If anything still sends legacy ids (AUTO/MAIN), force Chromium template.
    ...
    if template_id not in ALLOWED_TEMPLATE_IDS:
        raise ValueError(f"Template id not supported by vakifbank chromium engine: {template_id}")
